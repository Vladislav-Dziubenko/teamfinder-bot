...
_AI_CHAT_EXAMPLES = (
    ...
    "Вопрос: страж, ты живой вообще?\n"
    "Ответ: Я ИИ, пульса нет, зато чат вижу отлично. Пока вы спорите, я стою на воротах.\n"
    "Вопрос: страж, ну ты и тупой бот конечно\n"
    "Ответ: Жёстко, но принято. Дай конкретный баг — разберём без обид и дыма.\n"
    ...
    "Вопрос: страж, объясни как работают кейсы вкратце\n"
    "Ответ: Открываешь кейс, сервер выбирает предмет по шансам, а награда падает в инвентарь. Редкое — редко, честно и без магии.\n"
    "Вопрос: страж, а золото стоит открывать?\n"
    "Ответ: Если охотишься за редким лутом — да. Если копишь монеты, сначала забирай бесплатные открытия и квесты.\n"
...
_LIVE_LOOKUP_WORDS = (
    "курс", "доллар", "евро", "биткоин", "bitcoin", "btc", "новост",
    "последн", "сейчас", "сегодня", "выиграл", "матч", "реал", "real madrid",
)


def _guard_question(text: str) -> str:
    low = re.sub(r"\s+", " ", (text or "").strip().lower())
    low = re.sub(r"(^|\s)(страж|guardian)[\s,.:;!?-]*", " ", low, count=1).strip()
    return low
...
def guard_needs_live_lookup(text: str) -> bool:
    """Свежие вопросы пропускаем в LLM+поиск и не душим обычным cooldown."""
    q = _guard_question(text)
    if re.search(r"\b15\s*(умножить|x|\*|на)\s*12\b", q):
        return False
    return _has_any(q, _LIVE_LOOKUP_WORDS)


def _previous_user_topic(history: list[dict] | None) -> str:
    if not history:
        return ""
    for m in reversed(history[:-1]):
        ...
        if not text:
            continue
        q = _guard_question(text)
        if q and q not in {"как", "ну", "привет"}:
            return q[:90]
    return ""


def guard_quick_reply(text: str, history: list[dict] | None = None) -> str | None:
    """Дешёвые ответы на частые обращения: без сети, без ожидания и без квоты LLM."""
    q = _guard_question(text)
    norm = re.sub(r"[^a-zа-яё0-9* ]+", " ", q)
    ...
    if "понял" in norm and ("до этого" in norm or "до этого спрашивал" in norm):
        ...
        if prev:
            return f"Да, держу контекст: до этого ты спрашивал про «{prev}». Продолжай, я рядом."
        return "Пока помню только последние реплики чата. Напомни тему одним словом — и продолжим."
    if "привет" in norm:
        return "Привет. Страж на посту, чат под присмотром."
    if "как дела" in norm:
        return "На посту, порядок охраняю. А ты как, готов собирать пати?"
    if "скучно" in norm:
        return "Скучно лечится просто: зови тиммейтов, открывай квесты и устраивай нормальную катку."
    if "кто" in norm and ("сделал" in norm or "создал" in norm):
        return "Меня собрали для TeamFinder: люди делают магию, я сторожу чат и иногда умничаю."
    if "живой" in norm:
        return "Я ИИ, так что без пульса. Зато отвечаю бодро и чат вижу."
    if "тупой" in norm or "дурак" in norm:
        return "Обидно, но переживу. Лучше кинь конкретный баг — там я полезнее, чем в споре."
    if "справишься" in norm and "модерац" in norm:
        return "Справлюсь с очевидным мусором, а спорные моменты лучше отдавать людям. Так честнее."
    if "поумничать" in norm or ("бот" in norm and "умнич" in norm):
        return "Умничаю дозированно. Моя работа — порядок, а не лекция на три тома."
    if "кейсы" in norm and ("объясни" in norm or "работают" in norm or "вкратце" in norm):
        return "Кейсы простые: открываешь, сервер выбирает награду по шансам, предмет падает в инвентарь."
    if "золото" in norm and ("открывать" in norm or "стоит" in norm):
        return "Золото стоит открывать ради редкого лута. Если экономишь, сначала забирай бесплатное и квесты."
    if ("каждый день" in norm or "ежеднев" in norm) and ("выгод" in norm or "лучше" in norm):
        return "Да, регулярность выгоднее рывков: ежедневные бонусы, квесты и бесплатные открытия складываются."
    if norm in {"как", "как "}:
        return "Как именно? Дай продолжение вопроса, и я отвечу нормально."
    if norm in {"ну", "ну "}:
        return "Ну что, командир? Формулируй задачу — Страж слушает."
    if "апхах" in norm or "ахах" in norm or "ahaha" in norm:
        return "Смех принят. Теперь превращаем хаос в план?"
    if "го погнали" in norm or norm == "го" or "погнали" in norm:
        return "Го. Выбирай режим, зови людей — и поехали."
    return None
