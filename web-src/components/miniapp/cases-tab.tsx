...
export function CasesTab({ onToast }: { onToast: (m: string) => void }) {
  ...
  async function handleOpenMulti(c: LootCase, count: number) {
    ...
    setMultiBusy(count)
    try {
      // Сервер обрабатывает до 100 открытий одной транзакцией. Один request_id
      // защищает повтор после сетевого таймаута от двойного списания валюты.
      ...
      const items = res.items ?? (res.item ? [res.item] : [])
      if (items.length) {
        if (!c.free) analytics.purchase(c.id, c.costStars, count)
        setMulti({ box: c, items, fair: res.fair })
      }
    } finally {
      openBusyRef.current = false
      setMultiBusy(null)
