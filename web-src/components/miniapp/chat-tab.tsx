    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center justify-between gap-2">
          <p className="truncate text-sm text-muted-foreground">{previewText(chat.lastText, lang, chat.lastIsVoice)}</p>
          {chat.unread > 0 && (
...
function ReplyQuote({ nick, text, targetDomId, lang }: { nick: string; text: string; targetDomId: string; lang?: string }) {
  return (
    <button
      type="button"
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-xs opacity-80">{previewText(text, lang ?? "ru") || "…"}</span>
      </span>
    </button>
  )
}
...
function ReplyBar({ replyTo, onCancel, lang }: { replyTo: ReplyTarget; onCancel: () => void; lang?: string }) {
  return (
    <div className="flex items-center gap-2 border-t border-border bg-card/85 px-3 py-2 backdrop-blur-xl">
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-xs text-muted-foreground">{previewText(replyTo.text, lang ?? "ru") || "…"}</span>
      </span>
      <button
        type="button"
        onClick={onCancel}
...
/** Текст превью чата в списке: сырой tg_sticker: и пустые войсы не светим. */
function previewText(raw: string, lang: string, isVoice?: boolean): string {
  const t = (raw ?? "").trim()
  if (t.startsWith("tg_sticker:")) return lang === "ru" ? "🎨 Стикер" : "🎨 Sticker"
  if (!t && isVoice) return lang === "ru" ? "🎤 Голосовое сообщение" : "🎤 Voice message"
  return t
}
...
function MessageReactions({
  ...
  /** Компактный режим (войсы): плюсик сидит ближе к сообщению. */
  compact?: boolean
}) {
    ...
    <div className={compact ? "relative mt-0.5 flex flex-wrap items-center gap-1" : "relative mt-1 flex flex-wrap items-center gap-1"}>
      ...
      {onReact && (
        <button
          ...
          className={compact
            ? "grid size-5 place-items-center rounded-full bg-muted text-[11px] leading-none active:scale-95"
            : "grid size-6 place-items-center rounded-full bg-muted text-xs active:scale-95"}
        >
          +
        </button>
      )}
...
const MessageBubble = React.memo(function MessageBubble({ message: m, mine, chatId, frame, peerNick, onReact }: { message: Message; mine: boolean; chatId: string; frame?: string; peerNick?: string; onReact?: (messageId: string, emoji: string) => void }) {
  ...
  return (
    <div id={`dmsg-${m.id}`} className={cn("flex", mine ? "justify-end" : "justify-start")}>
      ...
      >
        {dmReply && <ReplyQuote nick={dmReplyNick} text={dmReply.text} targetDomId={`dmsg-${dmReply.id}`} lang={lang} />}
...
export function ChatConversation({ chatId, player, role, onBack, clanMode, clanEmblem, onOpenProfile }: { chatId: string; player?: ChatPreview["player"]; role?: string; onBack: () => void; clanMode?: boolean; clanEmblem?: string; onOpenProfile?: (id: number) => void }) {
  ...
  return (
    <div className="fixed inset-x-0 top-0 bottom-[60px] z-50 mx-auto flex max-w-md flex-col bg-background pb-[env(safe-area-inset-bottom)]" style={{ isolation: "isolate" }}>
      ...
      {replyTo && canSend && <ReplyBar replyTo={replyTo} onCancel={() => setReplyTo(null)} lang={lang} />}
...
function GlobalChat({ onBack, onOpenProfile }: { onBack: () => void; onOpenProfile?: (id: number) => void }) {
  ...
  return (
    <div className="fixed inset-x-0 top-0 bottom-[60px] z-50 mx-auto flex max-w-md flex-col bg-background pb-[env(safe-area-inset-bottom)]" style={{ isolation: "isolate" }}>
      ...
      {replyTo && !meBanned && <ReplyBar replyTo={replyTo} onCancel={() => setReplyTo(null)} lang={lang} />}
...
const GlobalMsg = memo(function GlobalMsg({
  ...
  return (
    <div id={`gmsg-${msg.id}`} className={cn("group flex", mine ? "justify-end" : "justify-start")}>
      ...
      <TranslateLangPicker open={pickerOpen} onPick={doTranslate} onClose={() => setPickerOpen(false)} />
    </div>
  )
}, (prev, next) =>
...
function TranslateLangPicker({ open, onPick, onClose }: { open: boolean; onPick: (code: string) => void; onClose: () => void }) {
  ...
  return (
    <BottomSheet
      open={open}
      onClose={onClose}
      title={t("chat.translate_to")}
      autoFitKey={filtered.length}
      headerExtra={
