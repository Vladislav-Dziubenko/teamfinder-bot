...
while i < len(lines):
    ...
    # Fix: return web.json_response at indent 32 after if not await... should be 36
    ...
    # Fix: elif case_config.get("costCoins"): at 20 should be 20 (correct)
    # elif case_config.get("costCoins"): at 20
    # total_cost = ... at 24
    # if not await... at 24 should be 28
    # return at 28 should be 32
    
    # Fix: elif case_config.get("costCoins"): at 20 -> next line at 24
    # elif case_config.get("costCoins"): at 20 -> next at 24
    # if not await... at 24 -> should be 28
    # return at 28 -> should be 32
    
    # Fix elif case_config.get("costCoins"): at 20
    elif stripped.startswith('elif case_config.get("costCoins"):') and indent == 20:
        new_lines.append(line)
        i += 1
        # Next line total_cost at 24 - OK
        continue
    
    # Fix if not await db._adjust_currency_conn at 24 -> should be 28
    elif stripped.startswith('if not await db._adjust_currency_conn(') and indent == 24:
        new_lines.append(' ' * 28 + stripped + '\n')
        i += 1
        continue
    
    # Fix return at 28 after if not await at 24
    ...
    # Fix total_cost = ... at 24 under else
    elif stripped.startswith('total_cost = case_config["costStars"] * count') and indent == 24:
        new_lines.append(line)
        i += 1
        continue
