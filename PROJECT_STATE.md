...
| `clans` | Кланы: название, тег, эмблема, уровень, банк, сезон |
| `clan_members` | Участники (роль, вклад за сезон/всего, дата вступления) |
| `clan_quests` | Клановые квесты (тип, цель, прогресс, сроки) |
| `clan_points_log` | История очков: сырые `points` + зачтённые `granted` (антифрод) |
| `clan_seasons` / `clan_season_results` / `clan_season_members` | Сезоны, итоги, снапшоты вкладов |
| `clan_shop_items` / `clan_invites` | Магазин банка, инвайт-коды |
...
- Кланы: `POST /api/clans` (создать), `GET /api/clans/search|my|{id}`, `POST /api/clans/{id}/join|leave|kick|role|invites|settings`, `GET .../quests`, `POST .../quests/{qid}/claim`, `GET /api/clans/leaderboard`, `GET /api/clans/seasons/current|history`, `GET .../seasons/{ym}/members`, `GET|POST .../shop[/buy]`
- ... и другие
...
## Кланы (сентябрь 2026, ветка `feature/clans` → main)

Отдельная от `teams` (отряды для поиска) система прогрессии сообществ.
Один активный клан на юзера. Роли `leader/officer/member` действуют только
внутри клана и не дают staff-прав модерации (`roleRank` не тронут).

- **Очки:** кейс +10, квест +15, активный день +5, инвайт +25; кап 300/день.
  В `clan_points_log` пишется сырьё (`points`) + зачтено (`granted`) —
  лог источник правды для антифрода. 20% в банк клана.
- **Антифрод:** кап, флаг `suspicious` при >5× медианы клана, награды только
  из снапшота, стаж 7+ дней, `already_in_clan` без 500 (сейвпоинты).
- **Квесты:** дневной/недельные шаблоны, прогресс в транзакции начисления,
  забор офицером (compare-and-set, дабл-клейм невозможен).
- **Сезон:** календарный месяц, ролловер 1-го числа (идемпотентный, догоняет
  пропуски). Топ-10% по сумме + топ-3 по эффективности, выплаты топ-3 вклада.
- **Уровни:** пороги lifetime 0/1k/3k/8k/20k/50k; перки: +2 слота/ур. (кап +10),
  скидка только на coin-кейсы (кап −10%, звёзды не трогаем), +5% к очкам
  квестов (кап +25%). Уровень вычисляется из lifetime в точках потребления.
- **Банк/шоп:** покупки только лидеру (защита от слива офицером).
- **Чат клана:** `chat_messages` с `chat_id='clan-{id}'`, разбор id только через
  `Database.split_chat_id()`. Вкладка «Клан» в меню «Ещё» + `ChatConversation`
  в `clanMode` (без мута/блока/очистки).
- **v2 (roadmap):** клановые войны, рейд-босс, стрик-множитель, звания,
  голосование за траты банка.
